...
class RecipeController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        $Purchases = Recipe:: where('deleted_at', '=', null)
    ...
    public function store(Request $request)
    {
        ...
        \DB::transaction(function () use ($request) {
            ...
            $order->Ref = $request->recipe_name;//$this->getNumberOrder();
            ...
            $order->direct_cost_rate = $request->direct_cost_rate;
            $order->in_direct_cost_rate = $request->in_direct_cost_rate;
            $order->margin_cost = $request->margin_cost;
            $order->total_in_direct_cost = $request->total_in_direct_cost;
            ...
            RecipeDetail::insert($orderDetails);
        }, 10);

        return response()->json(['success' => true, 'message' => 'Recipe Created !!']);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($request, $id) {
            $role = Auth::user()->roles()->first();
            $view_records = Role::findOrFail($role->id)->inRole('record_view');
            $current_Purchase = Recipe::findOrFail($id);
                ...
                $old_purchase_details = RecipeDetail::where('recipe_id', $id)->get();
                ...
                foreach ($old_purchase_details as $key => $value) {
                    ...
                    if($value['purchase_unit_id'] !== null){
                        ...
                        if (!in_array($old_products_id[$key], $new_products_id)) {
                            $PurchaseDetail = RecipeDetail::findOrFail($value->id);
                ...
                foreach ($new_purchase_details as $key => $prod_detail) {
                    ...
                    if($prod_detail['no_unit'] !== 0){
                        ...
                        $orderDetails['dinein'] = $prod_detail['dinein'];
                        $orderDetails['takeaway'] = $prod_detail['takeaway'];
                        $orderDetails['delivery'] = $prod_detail['delivery'];
                        ...
                        if (!in_array($prod_detail['id'], $old_products_id)) {
                            RecipeDetail::Create($orderDetails);
                        } else {
                            RecipeDetail::where('id', $prod_detail['id'])->update($orderDetails);
                        }
                    }
                }
                ...
                $current_Purchase->update([
                    ...
                    'direct_cost_rate' => $request['direct_cost_rate'],
                    'in_direct_cost_rate' => $request['in_direct_cost_rate'],
                    'margin_cost' => $request['margin_cost'],
                    'total_in_direct_cost' => $request['total_in_direct_cost'],
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request) {
            ...
            // if (PurchaseReturn::where('recipe_id', $id)->where('deleted_at', '=', null)->exists()) {
            //     return response()->json(['success' => false , 'Return exist for the Transaction' => false], 403);
            // }else{
                ...
                // PaymentPurchase::where('recipe_id', $id)->update([
                //     'deleted_at' => Carbon::now(),
                // ]);
            // }
        ...
        \DB::transaction(function () use ($request) {
            ...
            foreach ($selectedIds as $purchase_id) {
                ...
                // }else{
                    ...
                    $current_Purchase = Recipe::findOrFail($purchase_id);
                    $old_purchase_details = RecipeDetail::where('recipe_id', $purchase_id)->get();
                    // Check If User Has Permission view All Records
                    if (!$view_records) {
                        // Check If User->id === current_Purchase->id
                        $this->authorizeForUser($request->user('api'), 'check_record', $current_Purchase);
                    ...
                    // PaymentPurchase::where('recipe_id', $purchase_id)->update([
                    //     'deleted_at' => Carbon::now(),
                    // ]);
                // }
            }
    ...
    public function show(Request $request, $id)
    {
        ...
        $role = Auth::user()->roles()->first();
        $view_records = Role::findOrFail($role->id)->inRole('record_view');
        $purchase = Recipe::with('details.product.unitPurchase')
    ...
    public function edit(Request $request, $id)
    {
        if (PurchaseReturn::where('purchase_id', $id)->where('deleted_at', '=', null)->exists()) {
            ...
            $purchase['direct_cost'] = $Purchase_data->direct_cost_rate;
            $purchase['in_direct_cost'] = $Purchase_data->in_direct_cost_rate;
            $purchase['margin_percentage'] = $Purchase_data->margin_cost;
            $purchase['total_in_direct_cost'] = $Purchase_data->total_in_direct_cost;
            ...
            foreach ($Purchase_data['details'] as $detail) {
                ...
                $data['dinein'] = $detail->dinein;
                $data['takeaway'] = $detail->takeaway;
                $data['delivery'] = $detail->delivery;
    ...
    public function Send_Email(Request $request)
    {
     ...
     public function duplicate(Request $request, $id)
     {
         try {
             ...
             $new_recipe->Ref = $recipe->Ref." (copy)";
             $new_recipe->date = Carbon::now();
             $new_recipe->direct_cost_rate = $recipe->direct_cost_rate;
             $new_recipe->in_direct_cost_rate = $recipe->in_direct_cost_rate;
             $new_recipe->margin_cost = $recipe->margin_cost;
             $new_recipe->total_in_direct_cost = $recipe->total_in_direct_cost;
             $new_recipe->tax_rate = $recipe->tax_rate;
             $new_recipe->TaxNet = $recipe->TaxNet;
             $new_recipe->discount = $recipe->discount;
             $new_recipe->shipping = $recipe->shipping;
             $new_recipe->GrandTotal = $recipe->GrandTotal;
             $new_recipe->statut = $recipe->statut;
             $new_recipe->notes = $recipe->notes;
             $new_recipe->save();

             foreach ($recipe_details as $detail) {
                 $new_detail = new RecipeDetail();
                 $new_detail->cost = $detail->cost;
                 $new_detail->dinein = $detail->dinein;
                 $new_detail->takeaway = $detail->takeaway;
                 $new_detail->delivery = $detail->delivery;
                 $new_detail->purchase_unit_id = $detail->purchase_unit_id;
                 $new_detail->TaxNet = $detail->TaxNet;
                 $new_detail->tax_method = $detail->tax_method;
                 $new_detail->discount = $detail->discount;
                 $new_detail->discount_method = $detail->discount_method;
                 $new_detail->recipe_id = $new_recipe->id;
                 $new_detail->product_id = $detail->product_id;
                 $new_detail->total = $detail->total;
