...
class OrderController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        $latestCounter = DB::table('counters')
        ->orderBy('open_counter_time', 'desc')
        ->first(); // Get the latest row
        $latestCounterTime = ($latestCounter && is_null($latestCounter->close_counter_time)) 
        ? $latestCounter->open_counter_time 
        : Carbon::now();
        ...
        // $latestCounterTime = DB::table('counters')->orderBy('open_counter_time', 'desc')->value('open_counter_time');
        $totalSales = DB::table('sales')
           ->whereNull('deleted_at')
           ->where('created_at', '>=', $latestCounterTime)
           ->count();
        ...
        $Sales = Sale::with('facture', 'client', 'warehouse','user','table')
            ->where('deleted_at', '=', null)
           
    ->where('created_at', '>=', $latestCounterTime)
            ->whereIn('preparation', ['placed', 'preparing'])
        ...
        $categorizedItems = DB::table('sales')
    ->join('sale_details', 'sale_details.sale_id', '=', 'sales.id')
    ->join('products', 'sale_details.product_id', '=', 'products.id')
    ->join('restaurant_tables', 'sales.table_id', '=', 'restaurant_tables.id')
    // ->select('sales.*', ) // Select required columns
    ->select(
    ...
    ->where('sales.statut','!=','hold')




//new
    ->where('sales.created_at', '>=', $latestCounterTime)
    
    ->whereIn('preparation', ['placed', 'preparing' , "ready","outfordelivery"])
    ...
    foreach ($categorizedItems as $item) {
        ...
        $groupedItems[$item['category_id']][] = $item;        
    
    }
    $categorizedItems =  $groupedItems;
        ...
        $Sales = $Filtred->offset($offSet)
            ...
            foreach ($sale_data['details'] as $detail) {
                ...
                $data['prepared_quantity'] = $detail->prepared_quantity;
                $items[] = $data;
                ...
                $item['id'] = $Sale['id'];
                $item['orderDate'] = $Sale['date'];
                $item['orderType'] = $Sale['orderType'];
                
                $item['orderTime'] = Carbon::parse($Sale['created_at'])->format('H:i:s');
                // $item['orderTime'] = new DateTime($Sale['created_at'])->format('H:i:s');
                $item['orderNo'] = $Sale['Ref'];
                $item['table'] = $Sale->table['tablename']??'None';
                $item['created_by'] = $Sale['user']->username;
                $item['status'] = $Sale['statut'];
                $item['orderStatus'] = $Sale['preparation'];
                $item['shipping_status'] =  $Sale['shipping_status'];
                $item['discount'] = $Sale['discount'];
                $item['shipping'] = $Sale['shipping'];
                $item['warehouse_name'] = $Sale['warehouse']['name'];
                ...
                $item['paymentReceived'] = number_format($Sale['paid_amount'], 2, '.', '');
                $item['paymentSent'] = number_format($item['GrandTotal'] - $item['paymentReceived'], 2, '.', '');//due
                $item['payment_status'] = $Sale['payment_statut'];
            ...
            // Query and process sales with preparation status "ready"
            $SalesReady = Sale::with('facture', 'client', 'warehouse', 'user')
            ->where('deleted_at', '=', null)
            ->whereIn('preparation',  ["ready","outfordelivery"])
            ...
            foreach ($sale_data['details'] as $detail) {
                $data = [];
                $unit = $detail->sale_unit_id ? Unit::find($detail->sale_unit_id) : Product::find($detail->product_id)->unitSale;

                $data['code'] = $detail->product_variant_id
                    ? ProductVariant::where('product_id', $detail->product_id)->where('id', $detail->product_variant_id)->first()->code
                    : $detail['product']['code'];
                $data['name'] = $detail->product_variant_id
                    ? '[' . ProductVariant::where('product_id', $detail->product_id)->where('id', $detail->product_variant_id)->first()->name . ']' . $detail['product']['name']
                    : $detail['product']['name'];
                $data['quantity'] = $detail->quantity;
                $data['isPrepared'] = false;
                $data['id'] = $id;
                ...
                $data['DiscountNet'] = $detail->discount_method == '2' ? $detail->discount : $detail->price * $detail->discount / 100;
                $tax_price = $detail->TaxNet * (($detail->price - $data['DiscountNet']) / 100);
                $data['Net_price'] = $detail->tax_method == '1'
                    ? $detail->price - $data['DiscountNet']
                    : ($detail->price - $data['DiscountNet']) / (($detail->TaxNet / 100) + 1);
                $data['taxe'] = $detail->tax_method == '1' ? $tax_price : $detail->price - $data['Net_price'] - $data['DiscountNet'];
            ...
            $data3[] = [
                ...
                'orderTime' => Carbon::parse($Sale['created_at'])->format('H:i:s'),
                'orderNo' => $Sale['Ref'],
                'created_by' => $Sale['user']->username,
                'payment_status' => $Sale['payment_statut'],
                'status' => $Sale['statut'],
                'orderStatus' => $Sale['preparation'],
                'shipping_status' => $Sale['shipping_status'],
                'GrandTotal' => number_format($Sale['GrandTotal'], 2, '.', ''),
                'items' => $items
            ];
        }
