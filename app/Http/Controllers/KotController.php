...
class KotController extends BaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($request, $id) {
            ...
            if ($current_Sale['payment_statut'] == 'paid' && $request['status'] == 'completed') {
            ...
            foreach ($sale_details as $key => $value) {
                if( $request['status'] == 'ready' ){
                    $value->update([
                        'prepared_quantity' => $value->quantity,
                    ]);
                }
    ...
    public function Print_Invoice_POS(Request $request, $id)
    {
        ...
        $item['Ref'] = $sale->Ref;
        $item['date'] = $sale->date;
        $item['time'] = Carbon::parse($sale->created_at)->format('H:i:s');
    ...
    public function create(Request $request)
    {
      ...
      public function edit(Request $request, $id)
      {
        if (SaleReturn::where('sale_id', $id)->where('deleted_at', '=', null)->exists()) {
          ...
          $sale['tax_rate'] = $Sale_data->TaxNet;
          $sale['paid_amount'] = $Sale_data->paid_amount;
          $sale['table_id'] = $Sale_data->table_id;
          $sale['orderType'] = $Sale_data->orderType;
          
          $sale['person_id'] = $Sale_data->person_id;
          ...
          foreach ($Sale_data['details'] as $detail) {
              ...
              if ($detail->product_variant_id) {
                ...
                $data['is_imei'] = $detail['product']['is_imei'];
                $data['imei_number'] = $detail->imei_number;
                $data['current'] = $detail->quantity + $stock;
                if ($detail->discount_method == '2') {
                    $data['DiscountNet'] = $detail->discount;
                } else {
                    $data['DiscountNet'] = $detail->price * $detail->discount / 100;
